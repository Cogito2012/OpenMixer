...
def _pad_to_largest_tensor(tensor, group):
    ...
    assert (
        world_size >= 1
    ), "comm.all_gather must be called from ranks within the given group!"
...
def setup_distributed(args, backend="nccl", port=None):
    ...
    dist.init_process_group(
        backend=backend,
        init_method="env://",
        timeout=datetime.timedelta(seconds=10800)  # three hours timeout
    )
